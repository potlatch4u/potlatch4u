 ...
 * 
 */
public class LocalCacheDBAdapter {

    private static final String LOG_TAG = LocalCacheDBAdapter.class.getSimpleName();

    private static final String DATABASE_NAME = "potlatch_cache.db";

    // ST:databaseTableVariableDeclaration:start
    static final String DATABASE_TABLE_GIFT = MoocSchema.Gift.TABLE_NAME;
    ...
    private static final String Gift_KEY_ID = MoocSchema.Gift.Cols.ID;
    private static final String Gift_LoginId = MoocSchema.Gift.Cols.LOGIN_ID;
    private static final String Gift_GiftId = MoocSchema.Gift.Cols.GIFT_ID;
    private static final String Gift_Title = MoocSchema.Gift.Cols.TITLE;
    private static final String Gift_Body = MoocSchema.Gift.Cols.BODY;
    private static final String Gift_ImageName = MoocSchema.Gift.Cols.IMAGE_NAME;
    private static final String Gift_ImageMetaData = MoocSchema.Gift.Cols.IMAGE_LINK;
    private static final String Gift_CreationTime = MoocSchema.Gift.Cols.CREATION_TIME;
    private static final String Gift_GiftTime = MoocSchema.Gift.Cols.GIFT_TIME;
    // ST:createShortVariables:finish

    // ST:databaseTableCreationStrings:start
    // SQL Statement to create a new database table.
    private static final String DATABASE_CREATE_GIFT = "create table "
            + DATABASE_TABLE_GIFT + " (" // start table
            + Gift_KEY_ID + " integer primary key autoincrement, " // setup
                                                                    // auto-inc.
            // ST:tableCreateVariables:start
            + Gift_LoginId + " INTEGER ," //
            + Gift_GiftId + " INTEGER ," //
            + Gift_Title + " TEXT ," //
            + Gift_Body + " TEXT ," //
            + Gift_ImageName + " TEXT ," //
            + Gift_ImageMetaData + " TEXT ," //
            + Gift_CreationTime + " INTEGER ," //
            + Gift_GiftTime + " INTEGER " //
    ...
    /**
     * constructor that accepts the replyMessenger to be associated with
     * 
     * @param _context
     */
    public LocalCacheDBAdapter(Context _context) {
    ...
    /**
     * constructor that accepts the replyMessenger to be associated with, and if this
     * DB should be created in memory only(non-persistent).
     * 
     * @param _context
     */
    public LocalCacheDBAdapter(Context _context, boolean memory_only_db) {
     ...
     * @throws SQLException
     */
    public LocalCacheDBAdapter open() throws SQLException {
        Log.d(LOG_TAG, "open()");
        try {
    ...
    public Cursor query(final String table, final String[] projection,
		...
		// TDO: Perform a query on the database with the given parameters
    ...
    private static class myDbHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL(DATABASE_CREATE_GIFT);
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            db.execSQL("DROP TABLE IF EXISTS " + DATABASE_TABLE_GIFT);
