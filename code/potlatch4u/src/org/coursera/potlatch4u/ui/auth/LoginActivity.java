...
public class LoginActivity extends AccountAuthenticatorActivity {
    ...
    private final Map<String, String> userToPassMap = new HashMap<String, String>();
    ...
    private class LoginTask extends AsyncTask<String, Void, Boolean> {
        ...
        public Boolean doInBackground(String... params) {
            ...
            // e.printStackTrace();
            // }
            if (!credentialsCorrect(user, pass)) {
                return false;
            }
        ...
        private boolean credentialsCorrect(String user, String providedPassword) {
            String correctPassword = userToPassMap.get(user);
            return correctPassword != null && correctPassword.equals(providedPassword);
        }
    ...
    private void returnLoginResult(boolean result) {
        if (result) {
            Toast.makeText(LoginActivity.this, R.string.login_succeeded, Toast.LENGTH_SHORT).show();
            publishIntent(result);
            finish();
        } else {
    ...
    private void publishIntent(boolean result) {
        Intent intent = new Intent();
        intent.putExtra(LOGIN_SUCCESS, result);
        setResult(RESULT_OK, intent);
    }

    private void initUserStore() {
        userToPassMap.put("admin", "pass");
        userToPassMap.put("user", "pass");
        userToPassMap.put("user1", "pass1");
        userToPassMap.put("user2", "pass2");
        userToPassMap.put("user3", "pass3");
        userToPassMap.put("user4", "pass4");
        userToPassMap.put("user5", "pass5");
    }
}
// ST:BODY:end 